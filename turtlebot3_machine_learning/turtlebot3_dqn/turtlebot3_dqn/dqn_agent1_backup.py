...
class DQNAgent1(Node):
    ...
    def __init__(self, stage_num, max_training_episodes):
        super().__init__('dqn_agent_robot1')
    ...
    def get_current_state(self):
        ...
        # Just do a dummy action=0 step to get the current state
        # The environment has already been reset by the coordinator
        req = Dqn.Request()
        req.action = 0  # Dummy action

        while not self.rl_agent_interface_client.wait_for_service(timeout_sec=1.0):
            self.get_logger().info(f'{self.robot_name} - Waiting for rl_agent_interface service...')
